...
void dfs(int node,int par){
    for(auto child: adj[node]){
        ...
        dfs(child,node);
        dp_m[node] += dp_m[child] + (dp_wm[child] + a[child]);
        dp_wm[node] += dp_wm[child] + a[child];
    }
}
...
void rerooting(int node,int par){
    for(auto child: adj[node]){
        if(child==par) continue;
        int subtreeW = dp_wm[child] + a[child];
        int node_val = dp_m[node] - subtreeW + (totalW - subtreeW);
        int old_dp_m_child = dp_m[child];
        dp_m[child] = node_val;
        ans = max(ans,node_val);
        rerooting(child,node);
        dp_m[child] = old_dp_m_child;
    }
}
