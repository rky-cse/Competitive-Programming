...
int factorial( int num, int mod){ if( ::fact_dp.size() == 0){ ::fact_dp.push_back( 1); } for( int i = ::fact_dp.size(); i <= num; i++){ ::fact_dp.push_back( ( ::fact_dp[ i - 1] * ( i % mod)) % mod); } return ::fact_dp[ num]; }
int combinate(int n, int r, int p){ if (n < r)  return 0LL; if (r == 0LL) return 1LL;return (factorial(n,p) * modInverse(factorial(r,p), p) % p *modInverse(factorial(n-r,p), p) % p) % p; }
int permutate(int n, int r, int p){ if (r == 0) return 1LL; return (factorial(n,p) * modInverse( factorial(n - r, p), p)) % p; }
mt19937_64 gen(chrono::system_clock::now().time_since_epoch().count());
int rng(int mn, int mx){uniform_int_distribution<int> dis(mn, mx);return dis(gen);}
...
// this is my sgt template https://cses.fi/paste/1c6ce2b91fae3887a7617e/
...
/****you should update this block according to requirements***************************/
    ...
    SegmentType identity_element=inf;// change this if required
    ...
    for (int i = 0; i < n; i++){
        ...
        if(i > 0) {
            ...
            while(low <= high){
                ...
                if(mid == i || sgt.query(mid, i-1) >= a[i]){
                    l = mid;
                    high = mid - 1;
                } else {
                    low = mid + 1;
        ...
        if(i < n - 1) {
            ...
            while(low <= high){
                ...
                if(mid == i || sgt.query(i+1, mid) > a[i]){
                    r = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
        ...
        int sr = modSub(pp[r+1], pp[i]);
        
       
        int sl = 0;
        if(l == 0)sl = pp[i];  

        else sl = modSub(pp[i], pp[l-1]);
        
        int c = (a[i] % mod) * modSub(((int)lc * sr) % mod,((int)rc * sl) % mod ) % mod;
        ans = (ans + c) % mod;
    }
