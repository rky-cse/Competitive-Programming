...
void rky_cse(){
    ...
    bool ok = true;
    for (int i = 1; i <= n; i++){
        if(f[i] != 2){ ok = false; break; }
    }
    if(!ok){ cout << "NO\n"; return; }
    
    vector<vector<int>> occ(n+1), g(n);
    for (int i = 0; i < n; i++){
        occ[d[i].F].push_back(i);
        occ[d[i].S].push_back(i);
    }
    for (int i = 1; i <= n; i++){
    ...
    for (int i = 0; i < n; i++){
        if(col[i] == -1){
            int c0 = 0, c1 = 0;
            queue<int> q; q.push(i); col[i] = 0; c0++;
            bool bip = true;
            while(!q.empty()){
                int u = q.front(); q.pop();
                for (int v : g[u]){
    ...
    for(auto p : comp){
        ...
        for (int i = 0; i <= n; i++){
            if(dp[i]){
                if(i+a <= n) ndp[i+a] = true;
                if(i+b <= n) ndp[i+b] = true;
            }
        }
        dp = ndp;
    }
