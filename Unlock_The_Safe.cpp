...
void rky_cse(){
    ...
    for(int i=0;i<n;i++){
        ...
        int mn=min({abs(a[i]-b[i]),abs(9-abs(a[i]-b[i]))});
        tot+=mn;
        int mx=max({abs(a[i]-b[i]),abs(9-abs(a[i]-b[i]))});
    ...
    if(tot<=k){
        for(int i=0;i<n;i++){
            int nt=tot+vp[i].S-vp[i].F;
            if(nt<=k and abs(nt-k)%2==0){
                cout<<"Yes"<<ln;
                return;
            }
