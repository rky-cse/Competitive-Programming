...
int fun(int i, int band, int st) {
    if(i == n) {
        return (st && __builtin_popcount(band) == k) ? 1 : 0;
    }
    if(dp[i][band][st]!=-1)return dp[i][band][st];
    int ans = 0;

    ans = (ans + fun(i+1, band, st)) % mod;
    int newBand = st ? (band & a[i]) : a[i];
    ans = (ans + fun(i+1, newBand, true)) % mod;
    return dp[i][band][st]=ans;
}
